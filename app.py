...
def fetch_data(key, type_mode, args):
    try:
        ...
        if type_mode == 'discord':
            ...
            # Safe Count Access
            try: count_txt = f"{d['approximate_member_count']:,}"
            except: count_txt = "---"
                
            try: online_txt = f"{d.get('approximate_presence_count',0):,} ONLINE"
            except: online_txt = ""
...
def render_compact(d, ai_msg, css, radius, bg_col):
    """Server Invite - 400x110 Compact Mode"""
...
def handler(key, mode="auto"):
    ...
    # Server Defaults: No foreground bobbing, forced compact unless style overrides
    if data['type'] == 'discord':
        fg_an = 'false'
        if style == 'hyper': style = 'compact' # auto switch to compact

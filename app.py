...
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/Universal-v15'}
...
def get_css(master_anim, bg_anim, fg_anim):
    """
    Advanced CSS Generator with Granular Animation Control.
    Fixed XML entity error (&amp;) in Font URL.
    """
    # 1. Base Imports
    ...
    # 2. Keyframes (Always define these so valid CSS, but restrict usage via classes)
    css += """
    ...
    # 3. Apply Classes based on Logic flags
    # Priority: if Master=False, everything is off. 
    # If Master=True, check BG/FG individually.
...
# 1. EASTER EGG (macOS 26 / Fluid OS)
def render_easteregg(d, ai_msg, css, radius):
    bg_element = ""
    # Use album art if available, otherwise generate Aurora Mesh
    if d['album_art']:
    ...
    # AI Chip logic (Hidden if no ai_msg)
    ai_html = ""
    if ai_msg:
        ai_html = f"""
        <g transform="translate(130, 20)" class="hover-panel-2">
    ...
    return f"""<svg width="480" height="180" viewBox="0 0 480 180" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs><style>{css}</style>
        ...
        <clipPath id="main"><rect width="480" height="180" rx="{radius}"/></clipPath>
      ...
      <g clip-path="url(#main)">{bg_element}<pattern id="grain" width="100" height="100" patternUnits="userSpaceOnUse"><rect width="1" height="1" fill="white" opacity="0.05"/></pattern><rect width="100%" height="100%" fill="url(#grain)"/></g>
      <rect x="2" y="2" width="476" height="176" rx="{radius}" fill="none" stroke="url(#rainbowBorder)" stroke-width="4" class="flow-border" opacity="0.8"/>
      ...
      <g transform="translate(25, 50)" class="hover-panel">
         ...
         <g clip-path="url(#squircle)">
            <image href="{d['avatar']}" width="100" height="100"/><path d="M 0,0 L 100,0 L 0,100 Z" fill="white" opacity="0.1"/>
         </g>
...
def handler(key, mode="auto"):
    ...
    # Parsing Settings
    # 1. Anim Logic
    master = args.get('animations', 'true')
    ...
    # AI Processing
    role = "roast" if roast else "hud"
    ft = f"{data['l1']} {data['l2']}"
    msg = consult_gemini(ft, data['name'], role, ai_on, data.get('type'))

    # CSSGen
    css = get_css(master, bg_an, fg_an)

    # Renderer Selector
    if style == 'easteregg':
        # Apply specific border radius default if not overridden for cleaner mac look
        if 'borderRadius' not in args: rad = '28' 
        svg = render_easteregg(data, msg, css, rad)
    elif style == 'cute':
        ...
        svg = render_hyper(data, msg, css, rad, bg)

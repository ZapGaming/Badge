...
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/Chillax-v19'}
...
def sanitize_xml(text):
    """
    CRITICAL FIX: Removes control characters and illegal XML entities
    that cause 'CharRef: invalid decimal value' errors.
    """
    if not text: return ""
    # Remove vertical tabs, null bytes, and other non-printable chars (0-31), allowing tab(9), newline(10), carriage(13)
...
def get_css(master_anim, bg_anim, fg_anim):
    # Base Fonts (Including 'Pacifico' for the Chillax script look)
    css = "@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@800&amp;family=Rajdhani:wght@500;700&amp;family=Fredoka:wght@400;600;700&amp;family=Pacifico&amp;family=Fira+Code:wght@500&amp;family=Outfit:wght@400;700;900&amp;display=swap');"
    ...
    /* Chillax Background Motion */
    @keyframes aur { 0%{filter: hue-rotate(0deg);} 100%{filter: hue-rotate(60deg);} }
    @keyframes slowPan { 0% {background-position: 0% 50%;} 50% {background-position: 100% 50%;} 100% {background-position: 0% 50%;} }
    @keyframes mesh1 { 0%{cx:50px;cy:50px} 50%{cx:400px;cy:150px} 100%{cx:50px;cy:50px} }
    @keyframes mesh2 { 0%{cx:400px;cy:150px} 50%{cx:50px;cy:50px} 100%{cx:400px;cy:150px} }
    """
    ...
    if bg_on:
        ...
        .aurora-anim { animation: aur 20s infinite alternate; }
        .mesh-ball-1 { animation: mesh1 20s infinite ease-in-out; }
        .mesh-ball-2 { animation: mesh2 25s infinite ease-in-out; }
        """
    ...
    try:
        ...
        if type_mode == 'user' or (type_mode == 'auto' and len(str(key)) > 15):
            ...
            return {
                ...
                "l1": sanitize_xml(l1)[:25], "l2": sanitize_xml(l2)[:30],
                "color": col, 
        ...
        # DISCORD INVITE (Fixed Sanitization)
        else:
...
# 1. STYLE: CHILLAX (Custom Vencord Replica)
def render_chillax(d, ai_msg, css, radius, bg_arg):
    # If no BG provided, use the Chillax art style colors (dark slate/nebula)
    bg = bg_arg if bg_arg else "18191c"
    
    # 1. Custom Background Logic (The Wallpaper)
    if d['album_art']:
        bg_art = f'<image href="{d["album_art"]}" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" opacity="0.3" filter="url(#heavyBlur)"/>'
    else:
        # Recreate the screenshot vibe: Blue/Green/Pink blobs
        bg_art = f"""
        <rect width="100%" height="100%" fill="#{bg}"/>
        <circle cx="50" cy="50" r="100" fill="#00CFFF" class="mesh-ball-1" opacity="0.2" filter="url(#blur)"/>
        <circle cx="430" cy="130" r="120" fill="#FF00FF" class="mesh-ball-2" opacity="0.15" filter="url(#blur)"/>
        """

    # 2. AI Chat Bubble (Chillax style)
    ai_element = ""
    if ai_msg:
        ai_element = f"""
        <g transform="translate(130, 95)" class="hover-panel" style="animation-delay:0.3s">
            <rect width="320" height="35" rx="8" fill="rgba(0,0,0,0.4)" stroke="rgba(255,255,255,0.1)"/>
            <text x="15" y="22" font-family="JetBrains Mono" font-size="11" fill="{d['color']}">{ai_msg}<tspan class="cursor">_</tspan></text>
        </g>
        """
    ...
    return f"""<svg width="480" height="150" viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      <g transform="translate(30, 25)">
         <!-- Status Ring -->
         <circle cx="50" cy="50" r="48" fill="none" stroke="{d['color']}" stroke-width="3" stroke-dasharray="20 10" opacity="0.6" class="disc-spin"/>
         <g clip-path="url(#circleAv)">
            <image href="{d['avatar']}" width="100" height="100"/>
         </g>
         <!-- Online Dot -->
         <circle cx="85" cy="85" r="10" fill="{d['color']}" stroke="#{bg}" stroke-width="3"/>
      </g>

      <!-- INFO STACK -->
      <g transform="translate(145, 30)">
         <!-- Name (Script Style) -->
         <text x="0" y="15" class="script-font" font-size="34" fill="white" filter="url(#shadow)">! {d['name']}</text>
         
         <!-- Badges/Decor (Visual only) -->
         <text x="220" y="12" font-size="18">🌸 🍦 🤍</text>

         <!-- Glass Status Pill -->
         <g transform="translate(-5, 30)">
            <rect width="320" height="28" rx="6" fill="rgba(255,255,255,0.1)"/>
            <text x="10" y="19" class="ui-font" font-weight="600" font-size="13" fill="#E0E0E0">
               <tspan fill="{d['color']}">></tspan> {d['l1']} {d['l2']}
            </text>
         </g>
      </g>
      ...
      <!-- Footer ID -->
      <text x="470" y="145" text-anchor="end" class="mono" font-size="9" fill="rgba(255,255,255,0.3)">ID :: {d['id']}</text>
    </svg>"""
...
def render_spotify(d, ai_msg, css, radius, bg):
    ...
    return f"""<svg width="480" height="150" viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      <clipPath id="cp"><rect width="480" height="150" rx="{radius}"/></clipPath><clipPath id="ac"><rect width="100" height="100" rx="8"/></clipPath>
      <filter id="bl"><feGaussianBlur stdDeviation="15"/></filter></defs>
      <rect width="100%" height="100%" rx="{radius}" fill="#{bg}"/>
      <g clip-path="url(#cp)"><image href="{img}" width="480" height="480" x="0" y="-165" opacity="0.3" filter="url(#bl)"/><rect width="100%" height="100%" fill="black" opacity="0.2"/></g>
      <g transform="translate(20, 20)">
         <g class="float"><g clip-path="url(#ac)"><image href="{img}" width="100" height="100"/></g><rect width="100" height="100" rx="10" fill="none" stroke="rgba(255,255,255,0.1)"/></g>
         <g transform="translate(115, 10)"><text class="u" font-size="10" font-weight="bold" fill="#bbb" letter-spacing="2">NOW PLAYING</text><text y="28" class="u" font-size="24" font-weight="bold" fill="white">{d['l1']}</text><text y="50" class="m" font-size="12" fill="{d['color']}">{d['l2']}</text><text y="70" class="m" font-size="10" fill="#999">User: {d['name']}</text>{ai_html}</g>
      </g>
      <g transform="translate(20, 135)"><rect width="440" height="4" rx="2" fill="rgba(255,255,255,0.15)"/><rect width="{p_width}" height="4" rx="2" fill="{d['color']}"/></g>
    </svg>"""
...
@app.route('/')
def home(): return "CHILLAX HYPERBADGE ONLINE V19"

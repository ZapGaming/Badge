...
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/Titan-v22'}
...
def sanitize_xml(text):
    """Prevents XML/SVG Crashes by removing invisible control chars and escaping symbols."""
    if not text: return ""
    text = str(text)
...
def calculate_rank(stars, forks):
    """GitHub Repo Tier Calculator"""
    try:
        s, f = int(stars), int(forks)
        score = s + (f * 2)
        if score > 5000: return "EX"
...
def get_css(master_anim, bg_anim, fg_anim):
    # XML Safe URL (Ampersands escaped)
    base = "@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@800&amp;family=Rajdhani:wght@500;700&amp;family=Fredoka:wght@400;600;700&amp;family=Pacifico&amp;family=Outfit:wght@400;700;900&amp;family=Fira+Code:wght@500&amp;display=swap');"
    ...
    if not m_on: return base + keyframes + " * { animation: none !important; transition: none !important; }"
...
def fetch_data(key, type_mode, args):
    try:
        ...
        if type_mode == 'discord':
            ...
            # Robust member count extraction
            try: l2 = f"{d['approximate_member_count']:,}"
            except: l2 = "HIDDEN"
            
            return {
                "type": "discord", "name": name,
                "l1": "MEMBERS", "l2": l2, "color": "#5865F2", 
        ...
        # --- 2. GITHUB ---
            ...
            if '/' in key:
                ...
                # REPO STATS
                rank = calculate_rank(d['stargazers_count'], d['forks_count'])
                return {
                    "type": "github", "name": sanitize_xml(d['name']),
                    "l1": f"TIER: {rank} REP", "l2": f"★ {d['stargazers_count']}  ⑂ {d['forks_count']}",
                    "color": "#FFF", 
                    "avatar": get_base64(d['owner']['avatar_url']),
                ...
                return {
                    "type": "github", "name": sanitize_xml(d['login']),
                    "l1": "REPOSITORIES", "l2": str(d['public_repos']),
                    "color": "#FFF", 
                    "avatar": get_base64(d['avatar_url']),
                    "album_art": None, "is_music": False, "progress": 0, "id": str(d['id'])
                }

        # --- 3. LANYARD USER (Default) ---
        else:
...
def render_chillax(d, ai_msg, css, radius, bg_arg):
    ...
    ai = f"""<g transform="translate(130, 95)" class="hover-panel"><rect width="320" height="30" rx="8" fill="rgba(0,0,0,0.6)" stroke="rgba(255,255,255,0.2)"/><text x="15" y="19" font-family="JetBrains Mono" font-size="10" fill="{d['color']}">{ai_msg}<tspan class="cursor">_</tspan></text></g>""" if ai_msg else ""
    ...
    return f"""<svg width="480" height="150" viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs><style>{css}.script-font {{ font-family: 'Pacifico', cursive; }}.ui-font {{ font-family: 'Fredoka', sans-serif; }}.mono {{ font-family: 'JetBrains Mono', monospace; }}</style>
      <clipPath id="c"><rect width="480" height="150" rx="{radius}"/></clipPath><clipPath id="av"><circle cx="50" cy="50" r="45"/></clipPath>
      <filter id="blur"><feGaussianBlur stdDeviation="35"/></filter><filter id="heavyBlur"><feGaussianBlur stdDeviation="10"/></filter><filter id="sh"><feDropShadow dx="0" dy="2" flood-color="black" flood-opacity="0.8"/></filter></defs>
      <g clip-path="url(#c)">{bg_art}<rect width="100%" height="100%" fill="rgba(255,255,255,0.02)"/></g>
      <g transform="translate(30, 25)"><circle cx="50" cy="50" r="48" fill="none" stroke="{d['color']}" stroke-width="3" stroke-dasharray="8 6" opacity="0.8" class="disc-spin"/><g clip-path="url(#av)"><image href="{d['avatar']}" width="100" height="100"/></g><circle cx="85" cy="85" r="9" fill="{d['color']}" stroke="#{bg}" stroke-width="3"/></g>
      <g transform="translate(145, 30)"><text x="0" y="15" class="script-font" font-size="34" fill="white" filter="url(#sh)">! {d['name']}</text><text x="220" y="12" font-size="18">🌸 🤍</text><g transform="translate(-5, 30)"><rect width="320" height="28" rx="6" fill="rgba(255,255,255,0.1)"/><text x="10" y="19" class="ui-font" font-weight="700" font-size="13" fill="white"><tspan fill="{d['color']}" font-weight="900">></tspan> {d['l1']} {d['l2']}</text></g></g>{ai}<text x="470" y="145" text-anchor="end" class="mono" font-size="9" fill="#777">ID: {d['id']}</text></svg>"""

# 2. EASTER EGG (macOS Fluid)

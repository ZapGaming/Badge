...
# ===========================
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/Lite-v24'}
...
def fetch_data(key, type_mode, args):
    try:
        ...
        # --- 1. DISCORD INVITE (Member Counts) ---
        if type_mode == 'discord':
            ...
            # Use Force Name or Member Count
            main_title = force_name if force_name else f"{d.get('approximate_member_count', 0):,}"
            
            return {
                "type": "discord",
                "name": sanitize_xml(main_title),
                "l1": "TOTAL MEMBERS",
        ...
        # --- 2. LANYARD (USER) ---
            ...
            display_name = force_name if force_name else sanitize_xml(dname)
...
def render_chillax(d, css, radius, bg_arg):
    ...
    if d['album_art']:
        bg_art = f'<image href="{d["album_art"]}" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" opacity="0.6" filter="url(#heavyBlur)"/><rect width="100%" height="100%" fill="black" opacity="0.4"/>'
    else:
    ...
    return f"""<svg width="480" height="150" viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs>
        <style>{css} .script-font {{ font-family: 'Pacifico', cursive; }} .ui-font {{ font-family: 'Fredoka', sans-serif; }} .mono {{ font-family: 'JetBrains Mono', monospace; }}</style>
        <clipPath id="card"><rect width="480" height="150" rx="{radius}"/></clipPath><clipPath id="circleAv"><circle cx="50" cy="50" r="45"/></clipPath>
        <filter id="blur"><feGaussianBlur stdDeviation="35"/></filter><filter id="heavyBlur"><feGaussianBlur stdDeviation="10"/></filter><filter id="tShadow"><feDropShadow dx="0" dy="2" stdDeviation="2" flood-color="#000" flood-opacity="0.8"/></filter>
      </defs>
...
def handler(key, mode="auto"):
    ...
    if style == 'chillax': svg = render_chillax(data, css, rad, bg)
    elif style == 'spotify': svg = render_spotify(data, css, rad, bg)
    elif style == 'easteregg': svg = render_easteregg(data, css, rad)
    elif style == 'cute': svg = render_cute(data, css, bg)
    elif style == 'terminal': svg = render_terminal(data, css, bg)
    elif style == 'pro': svg = render_pro(data, args)
    else: svg = render_hyper(data, css, rad, bg)

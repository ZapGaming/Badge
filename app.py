...
def consult_gemini(status_text, user_name, mode, enabled):
    ...
    try:
        ...
        p = f"System status for '{user_name}' who is '{status_text}'. Tech HUD style. Uppercase. Max 6 words."
        if mode == "roast":
            p = f"Roast '{user_name}' about '{status_text}'. Savage. Uppercase. Max 6 words."
        if mode == "royal":
            p = f"Announce '{user_name}' status '{status_text}' in a royal/medieval decree style. Uppercase. Max 6 words."
...
def fetch_data(key, type_mode, args):
    try:
        ...
        if type_mode == 'discord':
            ...
            title = args.get('name', 'MEMBERS') # Force name if param exists
            count = f"{d['approximate_member_count']:,}"
            
            return {
                "type": "discord", "name": sanitize_xml(title), 
                "l1": count, "l2": "",
                "color": "#5865F2", "avatar": get_base64(f"https://cdn.discordapp.com/icons/{g['id']}/{g['icon']}.png") if g.get('icon') else EMPTY,
        ...
        # 2. GITHUB
            ...
            if '/' in key:
                ...
                title = force_name if force_name else d.get('name', 'Repo')
        ...
        # 3. LANYARD USER
            ...
            final_name = sanitize_xml(force_name if force_name else base_name)

...
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/Monolith-v26'}
...
def sanitize_xml(text):
    if not text: return ""
    text = str(text)
    # Remove control chars (0-31) to prevent XML breakages
...
def get_css(master_anim, bg_anim, fg_anim):
    # CSS: All Fonts needed for all styles
    ...
    # Master switch must be true for sub-switches to work
    ...
    if bg_on:
        classes += ".drift{animation:d 40s linear infinite;transform-origin:center} .scroll-bg{animation:s 20s linear infinite} .scanline{animation:sl 4s linear infinite} .mesh-1{animation:m1 15s infinite ease-in-out alternate} .mesh-2{animation:m2 20s infinite ease-in-out alternate-reverse} .mesh-3{animation:m3 12s infinite ease-in-out} .flow-border{animation:bf 4s linear infinite;stroke-dasharray:200}"
...
def consult_gemini(status_text, user_name, mode, enabled, data_type):
    ...
    try:
        ...
        if mode == "roast":
            ...
            prompt = f"{ctx} for '{user_name}': '{status_text}'. Tech HUD style. Uppercase. Max 6 words."

        response = model.generate_content(prompt)
        text = sanitize_xml(response.text.strip().replace('"','').replace("'", "")).upper()[:45]
        CACHE[key] = text
        return text
    except:
        return "SECURE"
...
def fetch_data(key, type_mode, args):
    try:
        ...
        if type_mode == 'discord':
            ...
            # Special Logic: Title is MEMBER COUNT if name not forced
            count_val = f"{d.get('approximate_member_count', 0):,}"
            display_title = force_name if force_name else "MEMBERS"
        ...
        # --- 2. GITHUB ---
             ...
             if '/' in key: # Repo
                ...
                return {"type":"github", "name": sanitize_xml(force_name or d['name']), "l1": f"RANK: {rank}", "l2": f"★ {d.get('stargazers_count')} ⑂ {d.get('forks_count')}", "color":"#FFF", "avatar": get_base64(d['owner']['avatar_url']), "id": str(d['id']), "album_art":None, "is_music":False, "progress":0}
             else: # User
                r = requests.get(f"https://api.github.com/users/{key}", headers=HEADERS, timeout=4)
                d = r.json()
                return {"type":"github", "name": sanitize_xml(force_name or d['login']), "l1": "REPOS", "l2": str(d.get('public_repos')), "color":"#FFF", "avatar": get_base64(d['avatar_url']), "id": str(d['id']), "album_art":None, "is_music":False, "progress":0}
        ...
        # --- 3. LANYARD USER ---
            ...
            return {
                ...
                "l1": sanitize_xml(l1)[:35], "l2": sanitize_xml(l2)[:40],
                "color": col, "avatar": get_base64(f"https://cdn.discordapp.com/avatars/{u['id']}/{u['avatar']}.png"),
                "album_art": art, "is_music": is_music, "progress": progress, "id": u['id']
            }
    except: return None

# ===========================
#      RENDER ENGINES (7 Styles)
# ===========================

def render_compact(d, css, radius, bg_col):
    """Specific engine for Discord Server stats (Small 110px height)"""
    ...
    return f"""<svg width="400" height="110" viewBox="0 0 400 110" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <defs><style>{css}.mono {{font-family:'JetBrains Mono';}}.big{{font-family:'Outfit';font-weight:900}}</style><clipPath id="cp"><rect width="400" height="110" rx="{radius}"/></clipPath><clipPath id="av"><rect width="70" height="70" rx="14"/></clipPath><filter id="b"><feGaussianBlur stdDeviation="25"/></filter></defs>
      ...
      <g transform="translate(20, 20)">
         ...
         <g transform="translate(90, 5)"><text x="0" y="10" font-family="Rajdhani" font-weight="700" font-size="12" fill="#AAA" letter-spacing="2">{d['name']}</text><text x="0" y="45" class="big" font-size="38" fill="white" letter-spacing="-1">{d['l1']}</text><text x="0" y="62" class="mono" font-size="10" fill="{d['color']}">● {d['l2']}</text></g>
      </g></svg>"""
...
def render_easteregg(d, ai_msg, css, radius):
    bg_el = f'<image href="{d["album_art"]}" width="100%" height="100%" opacity="0.6" filter="url(#hb)" preserveAspectRatio="xMidYMid slice"/><rect width="100%" height="100%" fill="black" opacity="0.6"/>' if d['album_art'] else f'<rect width="100%" height="100%" fill="#050505"/><circle r="120" fill="{d["color"]}" class="mesh-1" opacity="0.6" filter="url(#fb)"/><circle r="150" fill="#5865F2" class="mesh-2" opacity="0.5" filter="url(#fb)"/><circle r="100" fill="#00FFFF" class="mesh-3" opacity="0.4" filter="url(#fb)"/>'
...
def handler(key, mode="auto"):
    ...
    if 'github.com' in key or '/' in key: target_mode = 'github'
    ...
    # ROUTING
    if data['type'] == 'discord' and style == 'hyper': 
        svg = render_compact(data, msg, css, rad, bg) # Force compact for servers
    elif style == 'chillax': svg = render_chillax(data, msg, css, rad, bg)
...
@app.route('/')
def home(): return "CHILLAX HYPERBADGE ONLINE V25"

...
CACHE = {} 
HEADERS = {'User-Agent': 'HyperBadge/v23-Refined'}
...
def fetch_data(key, type_mode, args):
    try:
        ...
        if type_mode == 'discord':
            ...
            # FORMAT: Title = Member Count, L1 = "Members", L2 = Online Count
            # If user provided &name=, use that. Otherwise use total members.
        ...
        # --- 2. GITHUB ---
            ...
            if '/' in key:
                ...
                rank = "SSS" if score > 5000 else "A" if score > 1000 else "B"
                title = force_name if force_name else d['name']
                return {
                    "type": "github", "name": sanitize_xml(title),
                    "l1": f"RANK: {rank}", "l2": f"⭐ {d['stargazers_count']}  🔱 {d['forks_count']}",
...
def render_chillax(d, ai_msg, css, radius, bg_arg):
    ...
    ai_element = f"""<g transform="translate(130, 95)" class="hover-panel" style="animation-delay:0.3s"><rect width="320" height="30" rx="8" fill="rgba(0,0,0,0.5)" stroke="rgba(255,255,255,0.15)"/><text x="15" y="19" font-family="JetBrains Mono" font-size="10" fill="{d['color']}">{ai_msg}<tspan class="cursor">_</tspan></text></g>""" if ai_msg else ""
    ...
    return f"""<svg width="480" height="150" viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      <filter id="blur"><feGaussianBlur stdDeviation="35"/></filter><filter id="heavyBlur"><feGaussianBlur stdDeviation="10"/></filter><filter id="tShadow"><feDropShadow dx="0" dy="2" stdDeviation="2" flood-color="#000" flood-opacity="0.8"/></filter></defs>
      ...
      <g transform="translate(145, 30)"><text x="0" y="15" class="script-font" font-size="34" fill="white" filter="url(#tShadow)">! {d['name']}</text><text x="220" y="12" font-size="18">🌸 🍦 🤍</text><g transform="translate(-5, 30)"><rect width="320" height="28" rx="6" fill="rgba(255,255,255,0.15)"/><text x="10" y="19" class="ui-font" font-weight="700" font-size="13" fill="#FFF" filter="url(#tShadow)"><tspan fill="{d['color']}" font-weight="900">></tspan> {d['l1']} {d['l2']}</text></g></g>{ai_element}
...
def handler(key, mode="auto"):
    ...
    # 5 Arguments needed here: text, name, role, enabled, data_type
...
@app.route('/')
def home(): return "CHILLAX ENGINE ONLINE V23"
